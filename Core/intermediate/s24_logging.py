...
class BlueBlob(Blob):
    ...
    def __add__(self, other_blob):
        logging.info("{} Blue collides with {}".format(datetime.datetime.now(), other_blob.color))
    ...
    for blue_id, blue_blob in blue_blobs.copy().items():
        for other_blobs in red_blobs, green_blobs, blue_blobs:
            for other_id, other_blob in other_blobs.copy().items():
                logging.debug("{} Checking {} & {} collision".format(datetime.datetime.now(), blue_blob.color, other_blob.color))
                if other_blob == blue_blob:
                    ...
                    if is_touching(blue_blob, other_blob):
                        ...
                        if other_blob.size <= 0:
                            logging.warning("{} {} deleted".format(datetime.datetime.now(), other_blob.color))
                            del other_blobs[other_id]
                        if blue_blob.size <= 0:
                            logging.warning("{} {} deleted".format(datetime.datetime.now(), blue_blob.color))
